...
print(folder)
tic1=time.time()
f, ax = plt.subplots(1,2, figsize=(args.figwidth, args.figheight))
...
for j,model in enumerate(models):
    ...
    for k,m in enumerate(models[model]['submodels']):
        ...
        label = f'{m}'
        ax[j].barh(new_y+offsets[k], vals, barWidth, label=label,color=colors[k],hatch=patterns[k]) 
        ax[j].set_xlabel(r"Sensitivity: $\frac{\partial\:\ln{S_{u}}}{\partial\:\ln{k}}$")
        ax[j].set_title(f'{model} {title}',fontsize=lgdfsz)
        ax[j].set_yticks(new_y, rxns, fontsize=6)
        ax[j].invert_yaxis()
