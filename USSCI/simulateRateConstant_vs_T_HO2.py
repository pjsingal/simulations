...
def generateData(model,m,reaction,P):
    ...
    k_list=[getRateConstant(gas,T,P,reaction) for T in T_list]
    data = zip(T_list,k_list)
...
for j,model in enumerate(models):
    ...
    for i, P in enumerate(P_list):
        name=f'K_vs_T_{collider}_{P}atm'
        ...
        for k,m in enumerate(models[model]['submodels']):
            ...
            sims=generateData(model,m,reaction,P)  
...
ax.set_ylabel(f'rate constant [cm^3/mol/s]')
toc1=time.time()
