...
for z,n in enumerate(models):
    ...
    for k,m in enumerate(models[n]):
        ...
        if args.xscale=='log' and args.yscale=='linear':
            ax.semilogx(np.multiply(tau,P), XNOdry, linestyle='solid', linewidth=lw,color=colors[k],label=f"{n}-{m}",zorder=zorders[k])
        elif args.xscale=='linear' and args.yscale=='log':
            ax.semilogy(np.multiply(tau,P), XNOdry, linestyle='solid', linewidth=lw,color=colors[k],label=f"{n}-{m}",zorder=zorders[k])
        elif args.xscale=='log' and args.yscale=='log':
            ax.loglog(np.multiply(tau,P), XNOdry, linestyle='solid', linewidth=lw,color=colors[k],label=f"{n}-{m}",zorder=zorders[k])
        elif args.xscale=='log' and args.yscale=='log':
            ax.plot(np.multiply(tau,P), XNOdry, linestyle='solid', linewidth=lw,color=colors[k],label=f"{n}-{m}",zorder=zorders[k])
