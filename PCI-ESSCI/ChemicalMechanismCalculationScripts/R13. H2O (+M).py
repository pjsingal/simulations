...
k_P = getKfromTroe(P_list,T_list,'H + OH (+M) <=> H2O (+M)','N2')
...
def plogFit(P_list,T_list,k_P,rxn):
    ...
    plt.legend(bbox_to_anchor=(1.05, 1.0), loc='upper left')
    plt.show()
plogFit(P_list,T_list,k_P,'H2O (+M) <=> H + OH (+M)')
...
plt.legend()
plt.title("Arrhenius fits for R13: H + OH (+M) <=> H2O (+M)")
plt.show()

# %% SPECIAL CASE 1: Assigning Ar parameters to ALL species
#H2O (+M) <=> H + OH (+M)
...
for species in speciesList:
    print(("- collider: '%s'\n  low-P-rate-constant: {A: 2.20621e-02, b: 4.74036e-01, Ea: -1.13148e+02}")%(species))
...
speciesList=['NH3', 'NO2', 'NO', 'N2O', 'H2', 'O2', 'O3', 'H', 'O', 'OH', 'HO2', 'H2O', 'H2O2', 'CO',
    ...
    'AR', 'HE', 'N2']
for species in speciesList:
    print(("- collider: '%s'\n  low-P-rate-constant: {A: 1.04529e-01, b: 5.50787e-01, Ea: -2.32675e+02}")%(species))
