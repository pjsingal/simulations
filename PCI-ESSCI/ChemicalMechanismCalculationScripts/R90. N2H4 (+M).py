...
k_P = getKfromTroe(P_list,T_list,'2 NH2 (+M) <=> N2H4 (+M)', 'AR')
...
def plogFit(P_list,T_list,k_P,rxn):
    ...
    plt.legend(bbox_to_anchor=(1.05, 1.0), loc='upper left')
    plt.show()
plogFit(P_list,T_list,k_P,'NH2 + NH2 (+M) <=> N2H4 (+M)')
...
plot_ratefit(np.array([300,1000,2000]), np.array([5.86,8.25,8.60]), pltcolours[2], "NH3")
plot_ratefit(np.array([300,1000,2000]), np.array([5.60,7.98,8.19]), pltcolours[3], "H2O")
plt.xlabel('Temperature (K)')
plt.ylabel('Rate Constant')
plt.legend()
plt.title("Arrhenius fits for R90: NH2 + NH2 (+M) <=> N2H4 (+M)")
plt.show()
...
for species in speciesList:
    print(("- collider: '%s'\n  low-P-rate-constant: {A: 2.17658e+01, b: -1.14715e-01, Ea: 4.19216e+02}")%(species))
